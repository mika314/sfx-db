...
// You can copy and use unmodified imgui_impl_* files in your project. See examples/ folder for examples
// of using this. If you are new to Dear ImGui, read documentation from the docs/ folder + read the top
// of imgui.cpp. Read online: https://github.com/ocornut/imgui/tree/master/docs
...
static void ImGui_ImplSDLRenderer_SetupRenderState()
{
  ...
  // FIXME: Technically speaking there are lots of other things we could backup/setup/restore during our
  // render process.
...
void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData *draw_data)
{
  ...
  // If the user has specified a scale factor to SDL_Renderer already via SDL_RenderSetScale(), SDL will
  // scale whatever we pass to SDL_RenderGeometryRaw() by that scale factor. In that case we don't want
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++)
  {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
    {
      ...
      else
      {
        ...
        SDL_Rect r = {static_cast<int>(clip_min.x),
        ...
        const float *xy = reinterpret_cast<const float *>(
          reinterpret_cast<const char *>(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, pos));
        const float *uv = reinterpret_cast<const float *>(
          reinterpret_cast<const char *>(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, uv));
#if SDL_VERSION_ATLEAST(2, 0, 19)
        const SDL_Color *color = reinterpret_cast<const SDL_Color *>(
          (const char *)(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.19+
#else
...
bool ImGui_ImplSDLRenderer_CreateFontsTexture()
{
  ...
  bd->FontTexture = SDL_CreateTexture(
    bd->SDLRenderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STATIC, width, height);
