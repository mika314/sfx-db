...
//  2026-03-12: Fixed invalid assert in ImGui_ImplSDLRenderer_UpdateTexture() if ImTextureID_Invalid is defined to be != 0, which became the default since 2026-03-12. (#9295)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-06-11: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed ImGui_ImplSDLRenderer_CreateFontsTexture() and ImGui_ImplSDLRenderer_DestroyFontsTexture().
//  2025-01-18: Use endian-dependent RGBA32 texture format, to match SDL_Color.
//  2024-10-09: Expose selected render state in ImGui_ImplSDLRenderer_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks.
//  2024-05-14: *BREAKING CHANGE* ImGui_ImplSDLRenderer3_RenderDrawData() requires SDL_Renderer* passed as parameter.
//  2023-05-30: Renamed imgui_impl_sdlrenderer.h/.cpp to imgui-impl-sdlrenderer.h/.cpp to accommodate for upcoming SDL3.
...
// Functions
static void ImGui_ImplSDLRenderer_SetupRenderState(SDL_Renderer* renderer)
{
...
void ImGui_ImplSDLRenderer_NewFrame()
{
    ...
    IM_ASSERT(bd != nullptr && "Context or backend not initialized! Did you call ImGui_ImplSDLRenderer_Init()?");
    IM_UNUSED(bd);
}

// Draw callbacks
static void ImGui_ImplSDLRenderer_DrawCallback_ResetRenderState(const ImDrawList*, const ImDrawCmd*) {} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to implement this way.

void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData* draw_data, SDL_Renderer* renderer)
{
    ...
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                ImGui_ImplSDLRenderer_UpdateTexture(tex);
    ...
    // Setup desired state
    ImGui_ImplSDLRenderer_SetupRenderState(renderer);

    // Setup render state structure (for callbacks and custom texture bindings)
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    ImGui_ImplSDLRenderer_RenderState render_state;
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                if (pcmd->UserCallback == ImGui_ImplSDLRenderer_DrawCallback_ResetRenderState)
                    ImGui_ImplSDLRenderer_SetupRenderState(renderer);
                else
...
void ImGui_ImplSDLRenderer_UpdateTexture(ImTextureData* tex)
{
    ImGui_ImplSDLRenderer_Data* bd = ImGui_ImplSDLRenderer_GetBackendData();
...
void ImGui_ImplSDLRenderer_CreateDeviceObjects()
{
}
...
bool ImGui_ImplSDLRenderer_Init(SDL_Renderer* renderer)
{
    ...
    io.BackendRendererName = "imgui-impl-sdlrenderer";
    io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
    io.BackendFlags |= ImGuiBackendFlags_RendererHasTextures;   // We can honor ImGuiPlatformIO::Textures[] requests during render.

    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    platform_io.DrawCallback_ResetRenderState = ImGui_ImplSDLRenderer_DrawCallback_ResetRenderState;
